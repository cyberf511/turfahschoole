...
export function emailApplicationReceived(studentName: string, oppTitle: string) {
  return {
    subject: `📩 تم استلام طلبك — ${oppTitle}`,
    html: `
      <div dir="rtl" style="font-family:'Cairo',sans-serif;max-width:500px;margin:0 auto;padding:24px;background:#f9fafb;border-radius:12px;">
        ...
        <p style="color:#374151;">لقد استلمنا بنجاح طلب تقديمك للفرصة التطوعية <strong>"${oppTitle}"</strong>.</p>
...
export function emailRejected(studentName: string, oppTitle: string, reason?: string) {
  return {
    subject: `❌ تحديث على طلبك — ${oppTitle}`,
    html: `
      <div dir="rtl" style="font-family:'Cairo',sans-serif;max-width:500px;margin:0 auto;padding:24px;background:#f9fafb;border-radius:12px;">
        <div style="text-align:center;margin-bottom:20px;">
          <span style="font-size:2rem;">📋</span>
        ...
        <p style="color:#374151;">مرحباً <strong>${studentName}</strong>،</p>
        <p style="color:#374151;">نأسف لإبلاغك بأن طلبك للمشاركة في فرصة <strong>"${oppTitle}"</strong> لم يُقبل.</p>
        ${reason ? `<p style="color:#6b7280;font-size:0.9rem;">السبب: ${reason}</p>` : ''}
